...
In our sample of spreadsheets (`r sample_size`) we see that a spreadsheet can have a number of sheets associated with it. We use `purrr` to apply a function element-wise to a list. We then unlist and count the number of sheets per workbook and plot as a histogram.  
...
```{r {r 2017-11-27-tsne_worksheets_joe_parks}}
# Plot a few of them.  red = value, beige = blank
image_inputs <-
  view_ranges %>%
  dplyr::rename(x = row, y = col, z = is_blank) %>%
...
```{r {r 2017-11-27-tsne_worksheets_vkaminski}, echo=FALSE}
# Plot a few of them.  red = value, beige = blank
image_inputs <-
  view_ranges %>%
  dplyr::rename(x = row, y = col, z = is_blank) %>%
...
```{r {r 2017-11-27-tsne_worksheets_sparse}, echo=FALSE}
# Plot a few of them.  red = value, beige = blank
image_inputs <-
  view_ranges %>%
  dplyr::rename(x = row, y = col, z = is_blank) %>%
...
Given some labels for our spreadsheets, we could employ the same strategy used for the MNIST digit data set and described in our last [blog post](http://www.machinegurning.com/rstats/tsne/), where we use Support Vector Machines to train on our t-SNE'ed data. This trains a model that we could use to predict labels for hitherto unseen spreadsheets.  
...
We're already part way there achieving this based on Duncan's original preparation of the spreadsheet data demonstrated in the `view_ranges` object. However, we lack the important variable to train on, the presence or absence of an error. Thus we would need an unsuprevised method for identifying cells which are unexpected (e.g. a harded coded number in a row full of formulas; the column totals).    

```{r}
dplyr::glimpse(view_ranges)
```
