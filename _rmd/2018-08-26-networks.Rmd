...
Soil science isn't widely regarded as being sexy but my [co-blogger](https://dspace.lib.cranfield.ac.uk/handle/1826/9298) might disagree with this assertion. To help promote this important discipline we use it is our basis for a [soil food web](https://en.wikipedia.org/wiki/Soil_food_web) and a model complex system (which we greatly simplify).    
...
This is trivial when using Docker. We can create a Neo4j instance that runs locally on our browser into which we can load our data.  
...
To answer our questions we need to remember that the main data structure of tidygraph is a `tbl_graph` which can be thought of as two tibbles with data for nodes and edges. Inspecting the `Edge Data` tibble values in the `from` and `to` we can see the row indices of the nodes stored in the `Node Data` tibble.  
